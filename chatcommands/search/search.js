...
module.exports = {
  ...
  async execute(message, args) {
    ...
    const shorthandMappings = {
      'ar': '1016896287007842415',
      'wa': '1016896349301653616',
      'sh': '1016896429584814240',
      'ma': '1016896363461611550',
      'as': '1016896402082758738',
    };

    let channelsToSearch = [];

    const channelId = channelMappings[channelOption] || shorthandMappings[channelOption];
    const channel = message.guild.channels.cache.get(channelId);

    if (channelOption === 'all') {
      channelsToSearch = Object.values(channelMappings).map(channelId => message.guild.channels.cache.get(channelId));
    } else {
      if (!channelId) {
    ...
    message.reply({
      ...
      const collector = message.channel.createMessageComponentCollector({
        filter: i => i.customId === 'pageSelect' && i.user.id === message.author.id,
        time: 120000,
      });
